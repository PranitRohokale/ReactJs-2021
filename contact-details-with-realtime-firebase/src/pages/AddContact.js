...
const AddContact = () => {
  ...
  const imagePicker = async (e) => {
    ...
    try {
      ...
      const resizedImage = readAndCompressImage(file, imageConfig);
      ...
      uploadTask.on(
        ...
        (snapshot) => {
          ...
          // if (progress == 100) {
            setIsUploading(false);
            toast("Uploded", { type: "success" });
          // }
        },
        (error) => {
          console.log("Uploading Error" + error);
          toast("Uploading Error", { type: "error" });
        },
        () => {
